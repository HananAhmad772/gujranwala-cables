...
// TikTok SVG icon with official black/teal colors
function TikTokIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.69a8.16 8.16 0 0 0 4.77 1.52V6.73a4.85 4.85 0 0 1-1-.04Z" fill="#000000" />
      <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-2.88 2.5 2.89 2.89 0 0 1-2.89-2.89 2.89 2.89 0 0 1 2.89-2.89c.28 0 .54.04.79.1V9.01a6.33 6.33 0 0 0-.79-.05 6.34 6.34 0 0 0-6.34 6.34 6.34 6.34 0 0 0 6.34 6.34 6.34 6.34 0 0 0 6.33-6.34V8.69a8.16 8.16 0 0 0 4.77 1.52V6.73a4.85 4.85 0 0 1-1-.04Z" fill="#00F2EA" fillOpacity="0.3" />
    </svg>
  );
}
...
export function ContactForm() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!form.fullName.trim() || !form.phone.trim() || !form.email.trim() || !form.description.trim()) {
      toast({
        title: locale === "en" ? "Please complete all fields." : "براہ کرم تمام فیلڈز مکمل کریں۔",
        variant: "error",
      });
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        toast({
          title: locale === "en" ? "Submitted successfully" : "کامیابی سے جمع ہوگیا",
          description: locale === "en" ? "Your quotation request has been received and will be reviewed shortly." : "آپ کی کوٹیشن درخواست موصول ہو گئی ہے اور جلد ہی جائزہ لیا جائے گا۔",
          variant: "success",
        });
        ...
        toast({
          title: locale === "en" ? "Failed to submit" : "جمع کرنے میں ناکامی",
          description: j.message || (locale === "en" ? "Please try again." : "دوبارہ کوشش کریں۔"),
          variant: "error",
        });
      }
    } catch {
      toast({
        title: locale === "en" ? "Network error" : "نیٹ ورک خرابی",
        description: locale === "en" ? "Please try again." : "دوبارہ کوشش کریں۔",
        variant: "error",
      });
    } finally {
      setLoading(false);
...
export function ReviewSubmissionForm() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!form.name.trim() || !form.review.trim()) {
      toast({
        title: locale === "en" ? "Please add your name and feedback." : "براہ کرم اپنا نام اور رائے درج کریں۔",
        variant: "error",
      });
      return;
    }
    ...
    try {
      ...
      formData.append("comment", form.review);
      ...
      if (res.ok) {
        toast({
          title: locale === "en" ? "Submitted successfully" : "کامیابی سے جمع ہوگیا",
          description: locale === "en" ? "Your review is pending approval and will appear after moderation." : "آپ کی رائے منظوری کے انتظار میں ہے اور بعد میں ظاہر ہوگی۔",
          variant: "success",
        });
  ...
  const displayRating = hoveredStar !== null ? hoveredStar : form.rating;
...
export function GoogleMapSection() {
  ...
  const address = settings?.address ?? "Shop No 50, New Bilal Ganj Market, Sheikhupura Road, Gujranwala";
    ...
    <section className="bg-muted/45 py-16">
      <div className="mx-auto grid max-w-7xl gap-8 px-4 lg:grid-cols-[0.85fr_1.15fr] lg:px-8">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-black tracking-normal">{locale === "en" ? "Find us at Shop No 50, New Bilal Ganj Market, Gujranwala." : "دکان نمبر 50، نیو بلال گنج مارکیٹ، گوجرانوالہ میں ہمیں تلاش کریں۔"}</h2>
          ...
          <div className="mt-6">
            ...
            <div className="flex gap-2">
              <a href={links.facebook} target="_blank" rel="noopener noreferrer" className="grid h-10 w-10 place-items-center rounded-md border border-white/15 bg-card transition hover:bg-accent" aria-label="Facebook">
                <FacebookIcon className="h-4 w-4" />
              </a>
              <a href={links.instagram} target="_blank" rel="noopener noreferrer" className="grid h-10 w-10 place-items-center rounded-md border border-white/15 bg-card transition hover:bg-accent" aria-label="Instagram">
                <InstagramIcon className="h-4 w-4" />
              </a>
              <a href={links.tiktok} target="_blank" rel="noopener noreferrer" className="grid h-10 w-10 place-items-center rounded-md border border-white/15 bg-card transition hover:bg-accent" aria-label="TikTok">
                <TikTokIcon className="h-4 w-4" />
              </a>
              <a href={links.youtube} target="_blank" rel="noopener noreferrer" className="grid h-10 w-10 place-items-center rounded-md border border-white/15 bg-card transition hover:bg-accent" aria-label="YouTube">
                <YoutubeIcon className="h-4 w-4" />
              </a>
            </div>
          </div>
        ...
        <div className="min-h-80 overflow-hidden rounded-lg border bg-card shadow-sm">
          <iframe
            title="Shop No 50, New Bilal Ganj Market, Sheikhupura Road, Gujranwala"
            src={mapSrc}
